    ...
    case GET -> Root / "start" =>
        ...
        array.toList match {
          ...
          case sender :: _ =>
            array.mkString(" ") match {
              case value
                  if value == notifyMessageOfWrongUser() || value == notifyMessageOfSending() =>
                MessageToMe(value)
              case _ =>
      ...
      def trySendTarget(ior: MyIor[F], message: String): F[Unit] =
        getPartsFromMessage(message) match {
          ...
          case (_, command, name) if command == "/g" && name == "" => dontSend()
