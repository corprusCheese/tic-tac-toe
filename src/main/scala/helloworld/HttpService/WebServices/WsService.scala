...
object WsService extends Http4sDsl[IO] with AbstractService[IO] {
  ...
  private def begin(): IO[Response[IO]] = {
    val toClient: Stream[IO, WebSocketFrame] =
      Stream.awakeEvery[IO](1.second).map(x => Text(s"x: $x"))
  ...
  private def echo(): IO[Response[IO]] = {
    ...
    val buildStreams: Queue[IO, Option[WebSocketFrame]] => (
    ...
    val buildSocket
      : (Stream[IO, WebSocketFrame], Pipe[IO, WebSocketFrame, Unit]) => IO[Response[IO]] =
      (to, from) => WebSocketBuilder[IO].build(to, from)
